        ...
        <div className={cn("size-full", className)}>
          ...
          >
            <AccordionPrimitive.Root
              ...
              onValueChange={(value) => {
                const first = value[0];
                if (typeof first === "string" && setExpandedItems) {
                  setExpandedItems((prev) => [...(prev ?? []), first]);
                }
              }}
              dir={dir as Direction}
            >
    ...
    return (
      ...
      >
        <AccordionPrimitive.Trigger
          ...
          aria-label={`Folder ${element}`}
          onClick={() => handleExpand(value)}
        >
          {expandedItems?.includes(value)
            ? openIcon ?? (
                <MemoizedLucideReactIcons.FolderOpenIcon className="size-4" />
              )
            : closeIcon ?? (
                <MemoizedLucideReactIcons.FolderIcon className="size-4" />
              )}
          <span>{element}</span>
        </AccordionPrimitive.Trigger>
    ...
    return (
      <AccordionPrimitive.Item value={value} className="relative">
        <AccordionPrimitive.Trigger
          ...
          dir={direction}
          disabled={!isSelectable}
          aria-label={`File ${value}`}
          aria-controls={`file-content-${value}`}
          className={cn(
        ...
        >
          {fileIcon ?? (
            <>
              <AccordionPrimitive.Content
                id={`file-content-${value}`} // Matching ID for aria-controls
                className="flex cursor-pointer items-center gap-1 rounded-md pr-1 text-sm duration-200 ease-in-out rtl:pl-1 rtl:pr-0"
              >
                <MemoizedLucideReactIcons.FileIcon className="size-4" />
              </AccordionPrimitive.Content>
            </>
          )}
          {children}
